...
class Service {
  ...
  registerTriggers(triggers) {
    ...
    for(let methodName in triggers) {
      ...
      if(typeof method == 'function') continue;
      triggerSource.registerCommandExecutor(methodName, method)
      r.table("triggers").insert({ id: methodName, list:[sourceName] },
        (id,ol,ne) => ol.merge({ triggers: ol('list').setUnion(ne('list')) }))
    }
    return Promise.all(promises)
  }

  triggerService(serviceName, command, byCommand) {
  ...
  trigger(command, byCommand) {
    return r.table('triggers').get(command.type).run(evs.db).then(
      triggerDesc => {
  ...
  emitEvents(listName, events, commandId) {
    ...
    return eventWriter.writeEvents(events, commandId)
  }
