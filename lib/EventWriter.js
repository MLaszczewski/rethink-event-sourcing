  ...
  writeEvents(events, commandId) {
    ...
    if(!this.started) {
      return new Promise((resolve, reject) => {
        eventsBucket.__writeCallback = resolve
        eventsBucket.__errorCallback = reject
        this.outputQueue = this.outputQueue.concat(eventsBucket)
      })
    }
    ...
    return new Promise((resolve, reject) => {
      r.table(this.tableName).orderBy(r.desc(r.row('id'))).limit(1)('id').run(this.service.db).then(
        ([ id ]) => {
          ...
          for(let bucket of out) {
            let writeCallback = bucket.__writeCallback
            let errorCallback = bucket.__errorCallback
            delete bucket.__writeCallback
            delete bucket.__errorCallback
            this.writeBucket(bucket).then(
              result => writeCallback && writeCallback(result)
            ).catch(
              error => errorCallback && errorCallback(result)
            )
          }
          resolve(true)
        }
